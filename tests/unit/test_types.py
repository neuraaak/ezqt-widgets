...
class TestTypeAliases:
    ...
    def test_should_accept_string_when_icon_source_extended_is_used(self) -> None:
        """Test that IconSourceExtended accepts string type."""
        icon: IconSourceExtended = "path/to/icon.png"
        assert isinstance(icon, str)

    def test_should_accept_none_when_icon_source_extended_is_used(self) -> None:
        """Test that IconSourceExtended accepts None type."""
        icon: IconSourceExtended = None
        assert icon is None

    def test_should_accept_qsize_when_size_type_is_used(self) -> None:
        """Test that SizeType accepts QSize type."""
        size: SizeType = QSize(24, 24)
        assert isinstance(size, QSize)

    def test_should_accept_tuple_when_size_type_is_used(self) -> None:
        """Test that SizeType accepts tuple type."""
        size: SizeType = (24, 24)
        assert isinstance(size, tuple)
        assert len(size) == 2

    def test_should_accept_qcolor_when_color_type_is_used(self) -> None:
        """Test that ColorType accepts QColor type."""
        color: ColorType = QColor(255, 0, 0)
        assert isinstance(color, QColor)

    def test_should_accept_string_when_color_type_is_used(self) -> None:
        """Test that ColorType accepts string type."""
        color: ColorType = "#FF0000"
        assert isinstance(color, str)

    def test_should_accept_qwidget_when_widget_parent_is_used(
        self, qt_application
    ) -> None:
    ...
    def test_should_accept_none_when_widget_parent_is_used(self) -> None:
        """Test that WidgetParent accepts None type."""
        parent: WidgetParent = None
        assert parent is None

    def test_should_be_int_when_animation_duration_is_used(self) -> None:
        """Test that AnimationDuration is int type."""
        duration: AnimationDuration = 200
        assert isinstance(duration, int)

    def test_should_have_expected_signature_when_event_callback_is_used(self) -> None:
    ...
    def test_should_have_expected_signature_when_value_callback_is_used(self) -> None:
    ...
    def test_should_export_all_types_when_module_is_loaded(self) -> None:
    ...
    def test_should_be_importable_from_main_module_when_types_are_used(self) -> None:
